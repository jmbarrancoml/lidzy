...
struct MenuBarView: View {
    @ObservedObject var model: LidzyModel
    var body: some View { VStack(alignment: .leading, spacing: 16) { HStack { Label("Lidzy", systemImage: "macbook").font(.headline); Spacer(); Text("\(Int(model.angle))°").monospacedDigit().foregroundStyle(.secondary) }; Toggle("Follow lid", isOn: $model.isEnabled); Picker("Style", selection: $model.style) { ForEach(EffectStyle.allCases) { Text($0.rawValue).tag($0) } }; Divider(); SettingsLink { Text("Settings…") }; Button("Quit Lidzy") { NSApplication.shared.terminate(nil) } }.padding(16).frame(width: 270) }
}
